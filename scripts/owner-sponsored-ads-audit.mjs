...
for(const token of ['Sponsored Advertising','Pause Kleenest Sponsored globally','Campaigns awaiting approval','Create Owner campaign','Placement CRUD','$5 Remove Ads disables AdMob/network inventory only','Choose & crop image','Image alt text','Image URL (optional alternative to upload)'])must(page.includes(token),`Sponsored Owner UI missing behavior: ${token}`);
for(const token of ['chooseOwnerSponsoredCreative','uploadOwnerSponsoredCreative','sponsored-ad-creatives','p_creative_mode','p_image_url','p_image_alt'])must(service.includes(token),`Sponsored Owner image creative service missing: ${token}`);
