...
export function EntityRow({title,subtitle,meta,onPress,children}:{title:string;subtitle?:string;meta?:string;onPress?:()=>void;children?:ReactNode}){const body=<View style={osCard}><Text style={{fontSize:16,fontWeight:'900',color:osColors.ink}}>{title}</Text>{subtitle?<Text style={{color:osColors.muted}}>{subtitle}</Text>:null}{meta?<Text style={{fontSize:12,color:osColors.muted}}>{meta}</Text>:null}{children}</View>;return onPress?<Pressable onPress={onPress}>{body}</Pressable>:body}

export function ActionSheetCard({title,body,children}:{title:string;body?:string;children:ReactNode}){return <View style={osCard}><Text style={{fontSize:17,fontWeight:'900',color:osColors.ink}}>{title}</Text>{body?<Text style={{color:osColors.muted,lineHeight:19}}>{body}</Text>:null}{children}</View>}

export function AuditTrailCard({reason,createdAt,actor}:{reason:string;createdAt?:string;actor?:string}){return <View style={{...osCard,padding:12}}><Text style={{fontWeight:'800',color:osColors.ink}}>{reason}</Text><Text style={{fontSize:12,color:osColors.muted}}>{[actor,createdAt].filter(Boolean).join(' · ')}</Text></View>}

export function DiagnosticDisclosure({title,value}:{title:string;value:unknown}){const[open,setOpen]=useState(false);return <View style={osCard}><Pressable onPress={()=>setOpen(v=>!v)}><Text style={{fontWeight:'900',color:osColors.green}}>{open?'Hide':'Show'} {title}</Text></Pressable>{open?<Text selectable style={{fontFamily:'monospace',fontSize:11,color:osColors.muted}}>{JSON.stringify(value,null,2)}</Text>:null}</View>}

export function PrimaryAction({label,onPress,disabled=false,danger=false}:{label:string;onPress:()=>void;disabled?:boolean;danger?:boolean}){return <Pressable disabled={disabled} onPress={onPress} style={{alignSelf:'flex-start',borderRadius:12,paddingHorizontal:14,paddingVertical:11,backgroundColor:danger?osColors.danger:osColors.green,opacity:disabled?.5:1}}><Text style={{color:'white',fontWeight:'900'}}>{label}</Text></Pressable>}
