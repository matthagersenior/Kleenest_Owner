...
async function authorizeOwnerSession(){
 ...
 if(!authorization.authorized){await getSupabaseClient().auth.signOut({scope:'local'});throw new Error('This account is not authorized for KleenestOS.');}
...
export default function OwnerSignIn(){
  ...
  async function signIn(){setBusy(true);setError(null);try{await signInOwner(email,password);await authorizeOwnerSession();router.replace('/')}catch(c){setError(messageOf(c))}finally{setBusy(false)}}
  ...
  return <ScrollView contentContainerStyle={{flexGrow:1,justifyContent:'center',padding:24,backgroundColor:'#f5f8f6'}} keyboardShouldPersistTaps="handled">
    <View style={{gap:14}}>
      ...
      <Text style={{color:'#607067',lineHeight:21}}>KleenestOS verifies your server authorization tier after authentication. Platform owners receive mutation controls; admins receive only the authority allowed by the backend.</Text>
      ...
      <Pressable disabled={busy||!email.trim()||!password} onPress={signIn} style={{backgroundColor:'#132b21',padding:15,borderRadius:14,opacity:(busy||!email.trim()||!password)?0.5:1}}><Text style={{color:'white',fontWeight:'900',textAlign:'center'}}>{busy?'Verifying KleenestOS authority…':'Sign in to KleenestOS'}</Text></Pressable>
    </View>
  </ScrollView>
}
