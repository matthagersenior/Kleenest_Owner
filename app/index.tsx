...
function nested(source:unknown,key:string){return source&&typeof source==='object'&&!Array.isArray(source)?(source as Record<string,unknown>)[key]:null;}
...
export default function OwnerControlPlane(){
 ...
 const integrity=nested(bundle?.snapshot,'integrity');const overview=nested(bundle?.snapshot,'overview');const historyConfig=nested(bundle?.history,'configuration');
 return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh}/>} contentContainerStyle={{padding:16,gap:16,paddingBottom:56}}>
  ...
  <View style={{backgroundColor:'#132b21',borderRadius:20,padding:18,gap:7}}><Text style={{color:'#bce4cf',fontSize:12,fontWeight:'800'}}>PRIVATE PLATFORM CONTROL PLANE</Text><Text style={{color:'white',fontSize:24,fontWeight:'800'}}>Kleenest Owner</Text><Text style={{color:'#d8e7df',lineHeight:20}}>Authoritative oversight across Consumer, Business, Fleet and Enterprise capabilities without replicating customer applications.</Text></View>
  ...
  <Link href="/access" asChild><Pressable style={{backgroundColor:'white',borderRadius:18,padding:16,gap:5}}><Text style={{fontSize:18,fontWeight:'800'}}>Business · Fleet · Enterprise access</Text><Text style={{color:'#627269',lineHeight:20}}>Inspect and change commercial tier, Fleet enablement and Enterprise capability through server-authoritative admin RPCs.</Text></Pressable></Link>
  ...
  <Section title="Platform snapshot" value={bundle?.snapshot}/><Section title="System overview" value={overview}/><Section title="Integrity state" value={integrity}/><Section title="Control-plane history" value={bundle?.history}/><Section title="Configuration state" value={historyConfig}/>
